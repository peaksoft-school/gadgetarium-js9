...
export const AddImageBannerWhenManyImages = ({
   deleteHandler,
   handleFileChange,
   containerImg,
   handleDrop,
...
const widthNestForLabel = (containerImg) => {
   if ([3].includes(containerImg)) {
      return '100%'
   }
   if ([1].includes(containerImg)) {
      return '10.938vw'
   }
   return '6.927vw'
}
...
const ContainerPhotoAndAddNewPhoto = styled('div')(({ containerImg }) => ({
   backgroundColor: 'rgba(144, 156, 181, 0.20)',
   borderRadius: '4px',
   width: '100%',
   '.box': {
      padding: '1.042vw',
